...
public class Main {
    public static void main(String[] args) {
        ...
        List<City> mostPopulatedCities = cities.stream()//Починаємо зі створення потоку із списку міст.
                .collect(Collectors.toMap(City::continent, Function.identity(),
                        (City c1, City c2) -> c1.population() > c2.population() ? c1 : c2))
                .values()
                .stream()
                .toList();
        // Вивід результату
        mostPopulatedCities.forEach(city -> System.out.println("Continent: " + city.continent() + ", Most Populated City: " + city.name() + ", Population: " + city.population()));
        ...
        Optional<City> mostPopulatedCapital = cities.stream()
                ...
                .max(Comparator.comparingInt(City::population)); // знаходимо максимальну густоту населення

        mostPopulatedCapital.ifPresent(city ->
                System.out.println("Most Populated Capital: " + city.name() +
                        ", Population: " + city.population()));
        System.out.println("5 task");

        Map<String, Optional<City>> mostPopulatedCapitalsByContinent = cities.stream()
                .filter(City::isCapital) // відфільтровуємо лише столиці
                .collect(Collectors.groupingBy(City::continent,
...
// Вивід результатів
        ...
        minPopulationByCountry.forEach((country, city) ->
                System.out.println(country + ": " + city.map(City::name).orElse("Немає даних")));
